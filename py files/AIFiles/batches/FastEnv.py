...
class CellOracleSB3VecEnv(VecEnv):  # Inherit from SB3 VecEnv
    ...
    def __init__(self,
        ...
        self.period = 999999 #log the overal success rate
        self.step_counter = 0
        try:
        ...
        with open(oracle_path, 'rb') as f:
            ...
            print("loaded")
        print("all items in umap: " , self.oracle.adata)
        sc.pl.umap(self.oracle.adata, color='celltype', show=False, title='Cell types in UMAP space')
    ...
    def reset_for_inference(self, start_idx: int, target_name: str) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            print("Error during reset_for_inference:", e)
            traceback.print_exc()
            raise
    ...
    def _calculate_umap_distances_to_target(self, cell_indices: np.ndarray) -> np.ndarray:
        ...
        current_umap_coords = self.umap_coords_normalized[cell_indices]  # Get UMAP coordinates for the current cells
    ...
    def _create_info_dict(self, goal_reached: np.ndarray, truncated: np.ndarray, simulation_failed: np.ndarray,
        ...
        for i in range(self.num_envs):
            ...
            if is_done[i]:
                ...
                info_dict_per_env["goal_reached_periodic"] = self.goal_reached_periodic/ self.number_of_episodes_started_periodic if self.number_of_episodes_started_periodic > 0 else 0.0
    ...
    def plot_umap_with_goal_regions(
            ...
            label_y_offset: float = 0.3 # Controls how far above the circle the label appears
        ...
        for celltype, centroid_coord in centroids_to_plot.items():
            if celltype == "Megakaryocytes":
                label_y_offset = -0.3
            if celltype == "Caudal Mesoderm":
                label_y_offset = -0.3
            if celltype == "Reprogramming PGCs":
                label_y_offset = -0.3
            if celltype == "LP/Intermediate Mesoderm":
                label_y_offset = -0.3
    ...
    def get_periodic_number(self):
        return self.goal_reached_periodic / self.number_of_episodes_started_periodic if self.number_of_episodes_started_periodic > 0 else 0.0
