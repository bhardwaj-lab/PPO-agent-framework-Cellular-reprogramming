...
class Setup:
    ...
    def __init__(self, tf_dict:dict, scRNA_dat:sc.AnnData, cluster_name:str, embedding_name:str,output_dir:str,load_dir:str,load_from_file_cellOc:bool = True, load_from_file_links:bool = True):
    ...
    def _initialize_cell_oc(self) -> co.Oracle:
        ...
        inverse_dict = co.utility.inverse_dictionary(self.tf_dict)
        celloracle.addTFinfo_dictionary(inverse_dict)
        # check if X_pca exists
        if "X_pca" in self.scRNA_dat.obsm.keys():
            celloracle.perform_PCA_mod()
        else:
            celloracle.perform_PCA(n_components=50)
    ...
    def _construct_network_specific_grns(self) -> co.Links:
        ...
        if self.celloracle is not None:
            self.celloracle.fit_GRN_for_simulation(alpha=10,
                                      use_cluster_specific_TFdict=True)
            self.celloracle.to_hdf5(os.path.join(self.ouput_dir, 'fit_cellOC.celloracle.oracle'))
